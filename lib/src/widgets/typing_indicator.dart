...
class _TypingIndicatorState extends State<TypingIndicator> with TickerProviderStateMixin {
  ...
  Widget build(BuildContext context) => AnimatedBuilder(
        ...
        child: Row(
          mainAxisAlignment: widget.bubbleAlignment == BubbleRtlAlignment.right ? MainAxisAlignment.start : MainAxisAlignment.end,
          children: <Widget>[
            ...
            Container(
              ...
              decoration: BoxDecoration(
                borderRadius: InheritedChatTheme.of(context).theme.typingIndicatorTheme.bubbleBorder,
                color: InheritedChatTheme.of(context).theme.typingIndicatorTheme.bubbleColor,
              ),
              child: Wrap(
                spacing: 3.0,
                children: <Widget>[
                  AnimatedCircles(
                    circlesColor: InheritedChatTheme.of(context).theme.typingIndicatorTheme.animatedCirclesColor,
...
class TypingWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final sWidth = _getStackingWidth(widget.options.typingUsers);
    if (options.typingMode == TypingIndicatorMode.name) {
      return Flexible(
        child: Text(
    ...
    } else if (options.typingMode == TypingIndicatorMode.avatar) {
      return SizedBox(
        width: sWidth,
        child: AvatarHandler(
          context: context,
          author: widget.options.typingUsers,
          avatarBuilder: options.avatarBuilder,
        ),
      );
    } else {
      return Row(
  ...
  String _multiUserTextBuilder(List<types.User> author) {
    if (author.isEmpty) {
      ...
      return options.oneUserTypingText != null ? options.oneUserTypingText!(author.first) : '${author.first.firstName} is typing';
...
class AvatarHandler extends StatelessWidget {
  ...
  final Widget Function(types.User)? avatarBuilder;
  ...
  Widget build(BuildContext context) {
    if (author.isEmpty) {
      ...
      return Align(
        ...
        child: avatarBuilder != null ? avatarBuilder!(author.first) : TypingAvatar(context: context, author: author.first),
      );
    } else if (author.length == 2) {
      return Stack(
        children: <Widget>[
          avatarBuilder != null ? avatarBuilder!(author.first) : TypingAvatar(context: context, author: author.first),
          Positioned(
            left: 16,
            child: avatarBuilder != null ? avatarBuilder!(author[1]) : TypingAvatar(context: context, author: author[1]),
          ),
        ],
      );
    } else {
      return Stack(
        children: <Widget>[
          ...
          Positioned(
            ...
            child: CircleAvatar(
              radius: 13,
              backgroundColor: InheritedChatTheme.of(context).theme.typingIndicatorTheme.countAvatarColor,
              child: Text(
                '+${author.length - 2}',
                style: TextStyle(
                  color: InheritedChatTheme.of(context).theme.typingIndicatorTheme.countTextColor,
                ),
...
class AnimatedCircles extends StatelessWidget {
  ...
  Widget build(BuildContext context) => SlideTransition(
        ...
        child: Container(
          height: InheritedChatTheme.of(context).theme.typingIndicatorTheme.animatedCircleSize,
          width: InheritedChatTheme.of(context).theme.typingIndicatorTheme.animatedCircleSize,
          decoration: BoxDecoration(
            color: circlesColor,
...
class TypingIndicatorOptions {
  const TypingIndicatorOptions({
    ...
    this.oneUserTypingText,
    this.twoUsersTypingText,
    this.othersTypingText,
    this.avatarBuilder,
  });
  ...
  /// Text used for showing one user is typing.
  final String Function(types.User)? oneUserTypingText;

  /// Text used for showing two users are typing.
  final String Function(types.User, types.User)? twoUsersTypingText;

  /// Text used when other users are typing.
  final String Function(types.User, int)? othersTypingText;

  /// Builder for typing user avatar (which should be a circle widget of radius 13).
  final Widget Function(types.User)? avatarBuilder;
}
